...
export async function createApp() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    rawBody: true,
    bodyParser: true,
    logger: NexVoo.NexVoo.stable ? ['log'] : ['verbose'],
  });
  ...
  if (NexVoo.isSelfhosted && NexVoo.telemetry.enabled) {
    const mixpanel = await import('mixpanel');
    mixpanel.init(NexVoo.telemetry.token).track('selfhost-server-started', {
      version: NexVoo.version,
    });
  }
